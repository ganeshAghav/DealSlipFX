...
namespace FP.Common.ExceptionLogging
{
    #region Logger Class
    ...
    class Logger
    {
       ...
       // }
        ...
        public void IsLoggingSeperate()
        {
            ...
            {
                errorTypeValue = ConfigurationSettings.AppSettings["Information"];
                CheckLoggingCondition(errorTypeValue);
            }
            else if (ErrType == FPException.ErrorType.Warning)
            {
                errorTypeValue = ConfigurationSettings.AppSettings["Warning"];
                CheckLoggingCondition(errorTypeValue);
            }
            else
            {
                errorTypeValue = ConfigurationSettings.AppSettings["Error"];
                CheckLoggingCondition(errorTypeValue);
            }
        }
        ...
        public void XmlRead(Boolean bSqlError)
        {
            //if (!this.oFpCommon.CommonSettings.LoggingSettings.IsSeperate)
           /*    if(Convert.ToBoolean(ConfigurationManager.AppSettings["LogIsSeperate"]) == false)
               {
                 for (int iCntLogDestination = 0; iCntLogDestination < this.oFpCommon.CommonSettings.LoggingSettings.LogDestination.Length; iCntLogDestination++)
                 {
                     if (this.oFpCommon.CommonSettings.LoggingSettings.LogDestination[iCntLogDestination].Active)
                     {
                         string sDestinationValue = this.oFpCommon.CommonSettings.LoggingSettings.LogDestination[iCntLogDestination].value.ToString();
                         if (sDestinationValue == LogDestination.FILE.ToString())
                         {
                             this.FileNameFromConfig = this.oFpCommon.CommonSettings.LoggingSettings.LogDestination[iCntLogDestination].FileName.ToString();
                         }
                         if (!bSqlError)
                         {
            ...
            if (Convert.ToBoolean(ConfigurationManager.AppSettings["LogIsSeperate"]) == false)
            {
                string sDestinationValue = ConfigurationManager.AppSettings["LogDestination"];
                if (sDestinationValue == LogDestination.FILE.ToString())
                {
                    this.FileNameFromConfig = ConfigurationManager.AppSettings["FilePath"];
                }
                if (!bSqlError)
                {
                    this.CheckLoggingCondition(sDestinationValue);
